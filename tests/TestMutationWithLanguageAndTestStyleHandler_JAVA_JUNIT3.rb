...
class TestMutationWithLanguageAndTestStyleHandler_JAVA_JUNIT3 < Test::Unit::TestCase
    ...
    def setup
        ...
        newJAVA_HOMEWithJunit << "C:\\junit3.jar"
        envVarHash = {}
        envVarHash["JAVA_HOME"] = newJAVA_HOMEWithJunit.join( getOSEnvSeparator() )
        ENV.update( envVarHash )
        
        @pathToSourceFileJavaForTestData = @locationOfTestFiles + "/java/2DCLPUC/src/"
        ...
        @testsuiteJavaJunit3Style = "TestRectangle.java"
        @testsuiteJavaJunit3StyleWithError = "TestRectangleWithError.java"
        @testsuiteJavaJunit3StyleWithSyntaxError = "TestRectangleWithSyntaxError.java"
        ...
        @mutationCmdNonExistentTestSuite = "ruby #{@mutatorName} -s \"#{@pathToSourceFileJavaForTestData + @sourcefileForJava}\" -t \"#{@pathToJavaTestData + @nonExistentTestSuite}\" -l junit3"
        @mutationCmdTestSuiteNotReferringToMutatedSource = "ruby #{@mutatorName} -s \"#{@pathToSourceFileJavaForTestData + @sourcefileForJava}\" -t \"#{@pathToJavaTestData + @testsuiteNotReferringToMutatedSource}\" -l junit3"
        @mutationCmdHangingTestSuite = "ruby #{@mutatorName} -s \"#{@pathToSourceFileJavaForTestData + @sourcefileForJava}\" -t \"#{@pathToJavaTestData + @testsuiteThatHangs}\" -l junit3"
        @mutationCmdSleepingTestSuite = "ruby #{@mutatorName} -s \"#{@pathToSourceFileJavaForTestData + @sourcefileForJava}\" -t \"#{@pathToJavaTestData + @testsuiteThatSleeps}\" -l junit3"
        @mutationCmdJUnit3 = "ruby #{@mutatorName} -s \"#{@pathToSourceFileJavaForTestData + @sourcefileForJava}\" -t \"#{@pathToJavaTestData + @testsuiteJavaJunit3Style}\" -l junit3"
    end
...
Please create a test suite called '#{@pathToJavaTestData + @nonExistentTestSuite}' and modify its references from:
 ...
 to 
    ...
    def test_user_is_alerted_when_given_test_suite_does_not_exist_and_mutator_is_invoked_as_instance
        ...
        begin
            mutator = Mutator.new( @pathToSourceFileJavaForTestData + @sourcefileForJava, nonExistentTestSuite, 1, 1, LanguageAndTestStyleHandler_JAVA_JUNIT3.new( nonExistentTestSuite, localDebug ), localDebug )
            mutator.run!
...
Please modify its references from:
 ...
 to 
    ...
    def test_mutator_can_be_given_junit3_language_handler_as_object
        localDebug = @debug || false
        if localDebug then
            puts "Running '#{__method__}'"
        end
        
        mutator = Mutator.new( @pathToSourceFileJavaForTestData + @sourcefileForJava, @pathToJavaTestData + @testsuiteJavaJunit3Style, 1, 1, LanguageAndTestStyleHandler_JAVA_JUNIT3.new( @pathToJavaTestData + @testsuiteJavaJunit3Style, localDebug ), localDebug )
        begin
            mutator.run!
        rescue
            exceptionRaised = $!
    ...
    def test_mutator_can_handle_junit3_when_run_as_standalone
        if @debug then
            puts "Running '#{__method__}'"
        end
        
        stdoutAsString, stderrAsString = runCmd( @mutationCmdJUnit3 )
    ...
    def test_mutation_score_is_stable
        ...
        assertMutationScoreWhenStandalone( @mutationCmdJUnit3, expectedMutationScore, toleranceForMutationScore )
    end
    ...
    def test_language_handler_alerts_user_of_failing_test_before_mutation_analysis_begins
        ...
        mutator = Mutator.new( @pathToSourceFileJavaForTestData + @sourcefileForJavaWithError, @pathToJavaTestData + @testsuiteJavaJunit3StyleWithError, 1, 1, LanguageAndTestStyleHandler_JAVA_JUNIT3.new( @pathToJavaTestData + @testsuiteJavaJunit3StyleWithError, localDebug ), localDebug )
        assertOnFailingTests( mutator )

        mutator = Mutator.new( @pathToSourceFileJavaForTestData + @sourcefileForJavaWithSyntaxError, @pathToJavaTestData + @testsuiteJavaJunit3StyleWithSyntaxError, 1, 1, LanguageAndTestStyleHandler_JAVA_JUNIT3.new( @pathToJavaTestData + @testsuiteJavaJunit3StyleWithSyntaxError, localDebug ), localDebug )
